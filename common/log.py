...
def add_streamhandler(level, fmt):
    ...
    return add_handler(logging.StreamHandler, level, fmt, False)
...
def add_filehandler(level, fmt, filename , mode, backup_count, limit, when):
    ...
    #filename = filename + '_%s'%(os.getpid())
...
def set_logger(filename = None, mode = 'a', level='DEBUG:DEBUG',
               ...
               backup_count = 5, limit = 10*1024*1024, when = None):
...
file_path = '%s/logs/log%s.log' % (os.getcwd(),cur_date)
#set_logger(filename = file_path, mode = 'w')
#print file_path
set_logger(fmt = '[%(levelname)s] %(asctime)s [%(process)d] %(message)s ', filename = file_path, mode = 'w')